...
const index = () => {
  ...
  const { AuthReducer: { auth, isAuthentiCated }, RatingReducer: { consultantRatings } } = useAppSelector(state => state)
  useEffect(() => {
    if (isAuthentiCated) {
      ...
      <table className=" border border-gray/5 shadow-[0px_4px_10px_rgba(0, 0, 0, 0.1)]  rounded-md overflow-hidden w-full">
        <thead className="bg-[#FCFCFC] ">
          <tr>
            ...
            <td className="py-5 text-center">Comment</td>
        ...
        <tbody>
          {consultantRatings?.map?.((item) => {
            return (
              <tr className="border-t border-gray/5 " key={item?.id}>

                <td className="py-2  ">
                  <div>
                    <span className="block text-start">{item?.user
                      ?.name}</span>
                ...
                <td className="py-2  ">
                  <div>
                    <small className="block text-center">{item?.comment}</small>
