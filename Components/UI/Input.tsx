...
const Input = ({ className = "", invalid = false, ...rest }: propTypes) => {
    ...
    return (
        <div className="flex relative">
            <input {...rest} className={`border w-full px-3.5 py-[15px] focus:outline-none ${invalid ? "border-danger text-danger" : "border-[#DDDDDD]"} rounded-lg ${className} ${disabled?"bg-gray/20":""}`} />
